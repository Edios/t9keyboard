...
def _filter_func(doc):
    for exclude in [
        ...
        if doc.refname.startswith("t9keyboard." + exclude):
            return False
    return True
...
def generate():
    ...
    for output_file, html in _api_modules():
        ...
        with open(Path(output_file).absolute(), "wb") as f:
...
def main():
    ...
    parser.add_argument(
        "command", choices=["generate", "verify"], help="User choice to \"generate\" new documentation or \"verify\" if documentation is up to date"
    )
